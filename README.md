...
- **Interventions** — every control is a real fix Accra is debating, with one-click scenario bundles (*Today · GARID done · Full works*), qualitative state labels ("18% · choked" → "clear"), and changes that apply instantly, even mid-storm. The split-screen compare runs *your* chosen fixes against today's clogged baseline and labels the result.
- **Satellite mode** — a style toggle drapes a cloud-free Sentinel-2 true-color mosaic (baked into the file at build time, still fully offline) over the 3D terrain: the real Kotoka runway, Achimota Forest, the harbour, the rust roofscape — with the flood simulation running on top of the real city. Sand-table mode stays the default cartographic style.
- **City realism** — 22k minor streets, the airport runway/taxiways, railway lines, 6,900 green spaces tinted into the terrain, 74 searchable landmarks (Black Star Square, Makola, Korle Bu…), and buildings roofed with Accra's actual palette: rust corrugated sheets, zinc, plaster, dark browns, concrete for the tall ones.
...
- **Aerial chopper tour** *(hidden easter egg)* — a nod to the presidential flyover that surveyed the real floods. A classic cheat code launches a helicopter (built from primitives, spinning rotors, banking into its turns, cast shadow) that rides a chase-cam over the basin while the storm floods it beneath you, each neighbourhood named as you sweep across it. **Auto-pilot** flies a smooth loop through the hotspots, or switch to **Fly it myself** and pilot it directly (W/S forward·back, A/D turn, ↑/↓ climb·descend); Esc lands it. Try the most famous cheat code of all — the answer's in the browser console.
...
- Satellite imagery: contains modified Copernicus Sentinel-2 data (2022–2023), via the [AWS open registry](https://registry.opendata.aws/sentinel-2-l2a-cogs/).
