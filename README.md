...
- **Engineering mode** — take the engineer's seat and redesign the city against the water: paint new **storm drains**, dig **open channels** and **retention basins**, raise **levee walls**, or **clear buildings** off the floodplain (they leave the risk count — relocation, the AMA's actual buffer-zone policy). Every edit writes straight into the terrain and drainage textures the GPU solver reads, so the water reacts **live** — dam the Odaw and watch it back up 3.5 m behind your wall while the downstream channel runs dry. Your works are painted visibly onto the map as you build — orange drain lines, concrete channels, laterite levee embankments, dark excavated basins — with a pulse flash confirming every stroke. Brush size, an indicative works-cost ticker, undo, and one-click reset included.
